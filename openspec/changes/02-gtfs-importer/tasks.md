...
- [ ] Task 01: Create synthetic minimal GTFS test fixtures covering single-agency, multi-agency rejection, invalid agency refs, Case A/B calendars, non-consecutive sequences, >24h times, and same-feed duplicate records (`tests/fixtures/gtfs/`) <!-- id: 02-gi-01 -->
...
- [ ] Task 03: Install approved `csv-parse` dependency and implement CSV stream parser adapter adhering to contract specifications <!-- id: 02-gi-03 -->
- [ ] Task 04: Write failing tests and implement `agency.txt` Single-Agency Profile enforcement (single agency allowed with optional `agency_id`; multi-agency feed rejected with `UnsupportedMultiAgencyFeedError`) <!-- id: 02-gi-04 -->
- [ ] Task 05: Write failing tests and implement `routes.txt` Zod row schema and `routes.agency_id` single-agency referential validation <!-- id: 02-gi-05 -->
...
- [ ] Task 13: Write failing tests and implement Pass 1 Cross-File Referential Validator with Service ID Union (`route -> agency`, `trip -> route`, `trip -> service union`, `stop_time -> trip/stop`) <!-- id: 02-gi-13 -->
- [ ] Task 14: Write failing tests and implement same-feed duplicate PK rejection (`DuplicateFeedRecordError`) and same-feed conflicting payload detection (`ImportConflictError`) <!-- id: 02-gi-14 -->
